...
import { validateUrlInput } from "@/services/security/input-validator";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Analyze URL
    const result = await analyzeUrl(validation.data);
    ...
    const scan = await prisma.scan.create({
      data: {
        ...
        urlAnalysis: result.urlAnalysis
          ? {
              create: {
                url: result.urlAnalysis.url,
                domain: result.urlAnalysis.domain,
                isHttps: result.urlAnalysis.isHttps,
                hasValidSsl: result.urlAnalysis.hasValidSsl,
                sslIssuer: result.urlAnalysis.sslIssuer,
                domainAgeDays: result.urlAnalysis.domainAgeDays,
                registrar: result.urlAnalysis.registrar,
                ipAddress: result.urlAnalysis.ipAddress,
                country: result.urlAnalysis.country,
                httpStatus: result.urlAnalysis.httpStatus,
                redirectCount: result.urlAnalysis.redirectCount,
                redirectChain: result.urlAnalysis.redirectChain,
                securityHeaders: result.urlAnalysis.securityHeaders,
                hasLoginForm: result.urlAnalysis.hasLoginForm,
                hasPaymentForm: result.urlAnalysis.hasPaymentForm,
                dnsRecords: result.urlAnalysis.dnsRecords,
              },
            }
          : undefined,
      },
    ...
    logger.error("URL scan error", { error: String(error) });
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_ERROR", message: "An error occurred while analyzing the URL" } },
      { status: 500 }
    );
  }
}
